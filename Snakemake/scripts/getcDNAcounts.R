...
# dont remove pcr dups! input_bed_forward <- input_bed_forward %>% group_by(V1,V2,V4,V6) %>% filter(row_number()==1) # Get the unique cDNAs, FORWARD STRAND, so using V2
...
# dont remove pcr dups! input_bed_reverse <- input_bed_reverse %>% group_by(V1,V3,V4,V6) %>% filter(row_number()==1) # Get the unique cDNAs, REVERSE STRAND, so using V3
